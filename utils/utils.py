...
def load_batched_data(mfccPath, labelPath, batchSize, mode, level):
    ...
    return data_lists_to_batches([np.load(os.path.join(mfccPath, fn)) for fn in os.listdir(mfccPath) if not fn.startswith('.')],
                                 [np.load(os.path.join(labelPath, fn)) for fn in os.listdir(labelPath) if not fn.startswith('.')],
